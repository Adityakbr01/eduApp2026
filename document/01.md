# 🚀 EduApp Production Deployment Guide
...
## 1️⃣ Build Docker Images Locally (Optional / Manual)
...
> ⚠️ Local build is optional if you are using GitHub Actions for CI/CD.

---

## 2️⃣ Push Images to Docker Hub (Manual)
...
## 3️⃣ CI/CD with GitHub Actions (Recommended)
  ...
  build-and-push:
    ...
    steps:
      ...
      - name: Build & Push Video Worker Image
        uses: docker/build-push-action@v6
        with:
          context: ./video-pipline
          file: ./video-pipline/Dockerfile
          push: true
          tags: |
...
## 5️⃣ Production Deployment Flow
...
* **frontend** → Next.js standalone app (port `3000`)
* **backend** → Node.js / Express API (port `5000 → 3001`)
* **video-worker** → Background HLS/video processing worker
...
* 🔐 **Secrets** are loaded via `env_file` (never committed to Git)
* ♻️ **restart: always** ensures high availability
* ❤️ **Healthchecks** guarantee correct startup order
* 🌐 **Single private bridge network** for internal communication
* 🧹 **Log rotation enabled** to prevent disk overflow

This compose file is used directly by **GitHub Actions → EC2 deploy job**:
