...
import { useGetFeaturedCourses } from "@/services/courses/queries";
...
const MOCK_COURSES: CourseCardProps[] = [
  ...
  },
  {
    ...
    description:
      "Become a job-ready software engineer with our Cohort 2.0. An intensive program covering MERN stack development, DSA, and aptitude training.",
    hours: "150",
    hoursLabel: "Days",
    certified: true,
    mentorSupport: true,
  ...
  },
  {
    ...
    description:
      "Ace your coding interviews Master Java and DSA with our expert-led course, packed with interactive learning and real-world projects.",
    hours: "250+",
    hoursLabel: "Hours",
    certified: true,
    mentorSupport: true,
...
const HeroCourseSection = () => {
  ...
  const courses = useMemo(() => {
    ...
    return data.data.courses.map((course, index) => {
      const isEven = index % 2 !== 0; // 0=light(odd logic reversed?), let's stick to mock logic
      // Mock scheme: 0: light(reverse=true), 1: accent(reverse=false), 2: dark(reverse=true)
      // Let's cycle variants: light -> accent -> dark
      const variants: ("light" | "accent" | "dark")[] = [
        "light",
        "accent",
        "dark",
      ];
      const variant = variants[index % 3];
      const reverse = index % 2 === 0; // Alternate reverse

      return {
        title: course.title,
        description:
          course.title.length > "Data Science & Analytics with Gen AI".length
            ? undefined
            : course.description,
        hours: course.durationWeeks ? `${course.durationWeeks * 10}+` : "100+", // Approximation
        hoursLabel: "Hours",
        certified: true,
        mentorSupport: course.mentorSupport ?? true,
        price: course.pricing?.price.toString() || "0",
        originalPrice: course.pricing?.originalPrice.toString() || "0",
        imageUrl:
          getS3PublicUrl(course.thumbnail?.key || "") ||
          MOCK_COURSES[0].imageUrl,
        imageAlt: course.title,
        courseLink: `/course/${course.slug}`,
        variant: variant,
        reverse: reverse,
      };
    });
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ...
      const start = isMobile ? "top -40%" : "top -45%";
      const end = () =>
        isMobile
          ? `+=${window.innerHeight * (totalCards - 1)}` // Mobile: just enough to stack n-1 cards
          : `+=${window.innerHeight * (totalCards + 0.5)}`;
      ...
      cards.forEach((card, index) => {
        ...
        prevCards.forEach((prevCard, i) => {
          ...
          tl.to(
            prevCard,
            {
              scale: 1 - depth * (isMobile ? 0.0999 : 0.081),
              y: depth * (isMobile ? -42 : -45),
              duration: 1,
              ease: "none",
            },
            index - 1,
