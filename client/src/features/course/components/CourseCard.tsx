...
import deliveryModeConfig from "@/config/deliveryModeConfig";
...
function CourseCard({ course }: { course: ICourse }) {
  ...
  const mode = course.deliveryMode; // "Live" | "Offline" | "Recorded" | "Hybrid"
  const modeConfig = deliveryModeConfig[mode];
      ...
      >
        <div>
          ...
          <div className="relative">
            {modeConfig && (
              <h1
                className="bg-white flex items-center gap-1.5 sm:gap-2 w-max px-2 sm:px-3 py-0.5 rounded-md absolute right-2 sm:right-4 border-white shadow border-[0.67px] top-2 sm:top-4 uppercase font-manrope font-semibold text-sm sm:text-xl z-10"
                style={{ color: modeConfig.color }}
              >
                <span
                  className="h-1.5 sm:h-2 block rounded-full aspect-square animate-pulse"
